{ stdenv, makeWrapper, makeDesktopItem, fetchFromGitHub, gnused, pkgconfig
, gnome3, xorg, alsaLib, curl, freetype, libGL, libjack2 }:
let
...
in stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = [ pkgconfig makeWrapper gnused ];

  buildInputs = [ alsaLib curl freetype libGL libjack2 ] ++ (with xorg; [
    libX11
    libXext
    libXinerama
    libXrandr
